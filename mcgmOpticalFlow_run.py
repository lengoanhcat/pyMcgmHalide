...
def main():
    ...
    # filename = '/home/lpzal1/ipython_notebooks/Visual_Speech_Analysis/bbaf2n.mpg'
    filename = 'plaid.avi'
    ...
    inVidData = np.ndarray(shape=(im.shape[0],im.shape[1],im.shape[2],inVid.get_length()),dtype=np.uint8,order='F')
    ...
    os.system("gcc -shared -o mcgmOpticalFlow_halide.so mcgmOpticalFlow_halide.o")
    mcgmOpticalFlow = ctypes.cdll.LoadLibrary("./mcgmOpticalFlow_halide.so")
    ...
    divisionthreshold2 = ctypes.c_float(1e-25)
    speedthreshold = ctypes.c_float(1e-6)

    evalExpr = 'mcgmOpticalFlow.mcgmOpticalFlow_halide(input_buf_struct_p,noFrm,filterthreshold,divisionthreshold,divisionthreshold2,speedthreshold'
    for i in range(0,nargout):
    ...
    if error:
        ...
        for iF in range(0,np.uint16(noFrm)):
            ...
            rgbFrame = np.divide((normRgbFrame - normRgbFrame.min()),(normRgbFrame.max()-normRgbFrame.min()+10**-15))
