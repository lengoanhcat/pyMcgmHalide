...
def main():
    ...
    im = inVid.get_data(0);
    noRow = np.uint16(im.shape[0]/2)
    noCol = np.uint16(im.shape[1]/4)
    faceCenter = (im.shape[0]/2,im.shape[1]/2)

    # noFrm = inVid.get_length()
    noFrm = 200

    inVidData = np.ndarray(shape=(np.uint16(noRow/4),np.uint16(noCol/4),im.shape[2],noFrm),dtype=np.float32,order='F')
    for i,im in enumerate(inVid):
        tmp = np.float32(im)/255.0
        inVidData[:,:,:,i] = np.resize(tmp[
            np.uint16(faceCenter[0]-noRow/2):np.uint16(faceCenter[0]+noRow/2),
            np.uint16(faceCenter[1]-noCol/2):np.uint16(faceCenter[1]+noCol/2),:],
                                       (np.uint16(noRow/4),np.uint16(noCol/4),3))
    ...
    for i in range(0,nargout):
        outVidData.append(np.zeros(shape=(im.shape[0],im.shape[1],noFrm),dtype=np.float32,order='F'))
